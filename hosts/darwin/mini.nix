{
  ...
  home-manager.users.${userConfig.name}.home.packages = with pkgs; [
    ...
    (writeShellScriptBin "do_bak" ''
      #!/usr/bin/env zsh
      set -e
      restic --password-file ~/.config/restic-pw --repo rclone:sb:mini-bak backup ~/code ~/Documents ~/Desktop ~/Nextcloud /Volumes/West/Photos\ Library.photoslibrary/ /Volumes/West/KeilPlatte --skip-if-unchanged
